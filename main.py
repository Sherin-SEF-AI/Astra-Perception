...
class CameraThread(QThread):
    ...
    def __init__(self, camera_index=0, mode="ADAS", vehicle_mode="Scooter"):
        ...
        # AI Feature Flags (Controlled by GUI)
        self.enable_objects = True
        self.enable_drivable = True
    ...
    def run(self):
        ...
        while self._run_flag:
            ...
            if frame_idx % 30 == 0:
                now = time.time()
                self.avg_fps_val = 30.0 / (now - last_metric_time)
            ...
            if self.mode == "ADAS":
                ...
                if self.enable_drivable and self.drivable_detector and last_da_mask is not None:
                ...
                if alert:
                    self.voice_alert_signal.emit(alert, 15 if level <= ThreatLevel.LOW else (8 if level <= ThreatLevel.MEDIUM else 3), tid)
                
                if self.enable_objects:
                    for r in self.last_results:
                        box, dist, ttc, cid, oid = r['box'], r['distance'], r['ttc'], r['class_id'], r.get('id', '')
                        label = self.classes.get(cid, 'object')
                        color = (0, 255, 0)
                        if r.get('id') == tid:
                            color = (0, 0, 255) if level >= ThreatLevel.HIGH else (0, 255, 255)
...
class ADASMainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.main_layout.addLayout(self.upper_layout, stretch=6)
        ...
        self.upper_layout.addLayout(self.control_layout, stretch=1)
        ...
        self.main_layout.addWidget(QLabel("SYSTEM LOGS"), stretch=0)
        self.main_layout.addWidget(self.event_feed, stretch=1)
    ...
    def run_auto_scan(self):
        ...
        if local_cams:
            for combo_name, combo_data in self.camera_combos.items():
    ...
    def setup_camera_controls(self, name, default_idx, mode, label):
        ...
        if mode == "ADAS":
            toggles = QHBoxLayout()
            cb_obj = QCheckBox("Vision"); cb_obj.setChecked(True)
            cb_path = QCheckBox("Path"); cb_path.setChecked(True)
            cb_haz = QCheckBox("Surface"); cb_haz.setChecked(True)
            toggles.addWidget(cb_obj)
            toggles.addWidget(cb_path)
            toggles.addWidget(cb_haz)
    ...
    def start_cam(self, name, combo, mode, label, btn_on, btn_off, fps_lbl, status_lbl, cb_obj, cb_path, cb_haz):
        ...
        thread.log_signal.connect(self.event_feed.append)
    ...
    def update_metrics(self, fps, status, fps_lbl, status_lbl):
        ...
        if any(x in status for x in ["Warning", "Departure", "Brake"]):
